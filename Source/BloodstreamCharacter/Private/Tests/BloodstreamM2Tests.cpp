...
bool FM2InventoryTest::RunTest(const FString&){ UBloodstreamInventoryComponent* I=NewObject<UBloodstreamInventoryComponent>(); FBloodstreamOwnedItem Item; Item.InstanceId=FBloodstreamEntityId::NewId(); Item.ItemType=TEXT("M2.TestItem"); Item.Quantity=2; TestTrue(TEXT("add"),I->AddItem(Item)); TestFalse(TEXT("duplicate stable instance rejected"),I->AddItem(Item)); TestTrue(TEXT("remove quantity"),I->RemoveItem(Item.InstanceId,1)); const FBloodstreamOwnedItem* Remaining=I->FindItem(Item.InstanceId); TestNotNull(TEXT("remaining item"),Remaining); if(Remaining) TestEqual(TEXT("remaining quantity"),Remaining->Quantity,1); return true; }
...
bool FM2EquipmentTest::RunTest(const FString&){ UBloodstreamEquipmentComponent* E=NewObject<UBloodstreamEquipmentComponent>(); const auto Id=FBloodstreamEntityId::NewId(); TestTrue(TEXT("equip"),E->Equip(TEXT("Debug.Hand"),Id)); TestTrue(TEXT("query"),E->GetEquipped(TEXT("Debug.Hand"))==Id); TestTrue(TEXT("unequip"),E->Unequip(TEXT("Debug.Hand"))); return true; }
#endif
